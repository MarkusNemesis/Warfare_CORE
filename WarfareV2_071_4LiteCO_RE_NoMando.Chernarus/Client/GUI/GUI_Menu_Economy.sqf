...
while {alive player && dialog} do {	
	...
	if (mouseButtonUp == 0) then {
		...
		{
			// Markus - Disallow selling of base buildings after HQ is dead.
			if ( getDammage(Call Compile Format ["%1MHQ", sideJoinedText]) < 1) then
			{
				MenuAction = -1;
				...
				hint "You cannot sell base buildings whilst the HQ is destroyed";
			};
		};
	};

...
switch (true) do {
	...
	case WF_A2_CombinedOps: {
		...
		/* Mods */
		if (modACE) then {
			[] Call Compile preprocessFile 'Common\Config\Core_Gear\Core_ACE_G.sqf';
			[] Call Compile preprocessFile 'Common\Config\Core\Core_ACE_US_D.sqf';
			[] Call Compile preprocessFile 'Common\Config\Core\Core_ACE_US_W.sqf';
		};
	};
};
/* CORE SYSTEM - End */
...
//--- Neighbors
if (('WFBE_TOWNCONQUESTMODE' Call GetNamespace) > 0) then {[] Call Compile preprocessFile "Common\Init\Init_Neighbors.sqf"};
